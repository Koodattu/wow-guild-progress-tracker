...
export default function PickemsClient({ initialPickemId = null }: { initialPickemId?: string | null }) {
  const t = useTranslations("pickemsPage");
  const router = useRouter();
  const { user, isLoading: authLoading } = useAuth();

  const [pickems, setPickems] = useState<PickemSummary[]>([]);
  const [selectedPickemId, setSelectedPickemId] = useState<string | null>(initialPickemId);
  ...
  useEffect(() => {
    setSelectedPickemId(initialPickemId);
  ...
  const handlePickemSelect = useCallback(
    (id: string | null) => {
      ...
      setError(null);
      setSuccessMessage(null);
      router.push(id ? `/pickems/${encodeURIComponent(id)}` : "/pickems");
    },
    [router],
  );
